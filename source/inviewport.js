...
Object.prototype.inViewport = function inViewport(
   ...
   * @return boolean
   */
  const variableValidate = (xInput, yInput, measurementInput) => {
    var validVariables = true;

    if (x === null || typeof x === 'undefined') {
      validVariables = false;
      console.warn(
        'inViewport.js: missing x-axis viewport edge value.' +
    ...
    if (
      ...
      measurement = 'percentage';
      console.warn(
        'inViewport.js: invalid measurement setting.  Defaulting to percentage.' +
          '\nPlease see valid variable inputs at https://github.com/ianrogren/javascript-inViewport'
      );
    }
   ...
   * @return boolean
   */
  const verticalVisibility = (elementBounds, topBounds, bottomBounds, view) => {
    ...
    if (topBounds === true && bottomBounds === false) {
      ...
      verticalPercentage = Math.abs(
        (elementBounds.top - win.innerHeight) / elementBounds.height
      );
    } else if (topBounds === false && bottomBounds === true) {
      verticalShowing = elementBounds.bottom;
      verticalPercentage = Math.abs(
        elementBounds.bottom / elementBounds.height
      );
    ...
    if (
      (verticalPercentage >= y && measurement === 'percentage') ||
  ...
  const horizontalVisibility = (
    ...
    if (
      (horizontalPercentage >= y && measurement === 'percentage') ||
  ...
  const checkInView = () => {
    ...
    if (
      variableValidate(xValue, yValue, measurementType) ||
      horizontalVisibility(bounds, rightVisible, leftVisible, viewport) ||
      verticalVisibility(bounds, topVisible, bottomVisible, viewport)
    ) {
      return true;
    }
